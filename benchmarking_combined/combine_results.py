#!/usr/bin/env python3
"""
Combine and average results from multiple benchmark runs
"""
...
def create_combined_performance_chart(avg_results, std_results):
    ...
    precision_std = [std_results.loc[repo, 'Avg_Precision'] if repo in std_results.index else 0 
                    for repo in avg_results_sorted.index]
    recall_std = [std_results.loc[repo, 'Avg_Recall'] if repo in std_results.index else 0 
                 for repo in avg_results_sorted.index]
    f1_std = [std_results.loc[repo, 'Avg_F1'] if repo in std_results.index else 0 
             for repo in avg_results_sorted.index]
    accuracy_std = [std_results.loc[repo, 'Avg_Accuracy'] if repo in std_results.index else 0 
                   for repo in avg_results_sorted.index]
    ...
    time_std = [std_results.loc[repo, 'Avg_Processing_Time'] if repo in std_results.index else 0 
               for repo in avg_results_sorted.index]
...
def create_statistics_table(avg_results, std_results):
    ...
    for repo in avg_results.index:
        stats_data.append({
            ...
            'Avg_Precision': f"{avg_results.loc[repo, 'Avg_Precision']:.3f} ± {std_results.loc[repo, 'Avg_Precision']:.3f}" if repo in std_results.index else f"{avg_results.loc[repo, 'Avg_Precision']:.3f}",
            'Avg_Recall': f"{avg_results.loc[repo, 'Avg_Recall']:.3f} ± {std_results.loc[repo, 'Avg_Recall']:.3f}" if repo in std_results.index else f"{avg_results.loc[repo, 'Avg_Recall']:.3f}",
            'Avg_F1': f"{avg_results.loc[repo, 'Avg_F1']:.3f} ± {std_results.loc[repo, 'Avg_F1']:.3f}" if repo in std_results.index else f"{avg_results.loc[repo, 'Avg_F1']:.3f}",
            'Avg_Accuracy': f"{avg_results.loc[repo, 'Avg_Accuracy']:.3f} ± {std_results.loc[repo, 'Avg_Accuracy']:.3f}" if repo in std_results.index else f"{avg_results.loc[repo, 'Avg_Accuracy']:.3f}",
            'Avg_Processing_Time': f"{avg_results.loc[repo, 'Avg_Processing_Time']:.3f}s ± {std_results.loc[repo, 'Avg_Processing_Time']:.3f}s" if repo in std_results.index else f"{avg_results.loc[repo, 'Avg_Processing_Time']:.3f}s"
        })
...
def generate_final_report(avg_results, std_results):
    ...
    best_f1 = avg_results.loc[avg_results['Avg_F1'].idxmax()]
    best_precision = avg_results.loc[avg_results['Avg_Precision'].idxmax()]
    best_recall = avg_results.loc[avg_results['Avg_Recall'].idxmax()]
    best_accuracy = avg_results.loc[avg_results['Avg_Accuracy'].idxmax()]
    fastest = avg_results.loc[avg_results['Avg_Processing_Time'].idxmin()]
...
• **Best Precision:** {best_precision.name} ({best_precision['Avg_Precision']:.3f})
• **Best Recall:** {best_recall.name} ({best_recall['Avg_Recall']:.3f})
• **Best Accuracy:** {best_accuracy.name} ({best_accuracy['Avg_Accuracy']:.3f})
• **Fastest Processing:** {fastest.name} ({fastest['Avg_Processing_Time']:.3f}s)
...
• **Most consistent performer:** {avg_results.loc[avg_results['Run_Count'].idxmax(), 'Repository']} ({int(avg_results['Run_Count'].max())} runs)
