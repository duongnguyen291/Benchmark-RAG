#!/usr/bin/env python3
"""
Direct copy of evaluate.py code for benchmarking
"""
...
# Direct copy of functions from evaluate.py
...
def compute_sim_matrix(ex_nump, gt_nump):
    ...
    """
    # Use our own levenshtein_ratio function instead of Levenshtein.ratio
    matrix = cdist(ex_nump.reshape(-1, 1), gt_nump.reshape(-1, 1), lambda x, y: levenshtein_ratio(x[0], y[0]))
...
def run_benchmark():
    ...
    print("🚀 Running Direct evaluate.py Benchmark")
    print("=" * 60)
    ...
    # Save results
    output_dir = Path("direct_evaluate_results")
    ...
    try:
        ...
        plt.ylabel('Score')
        plt.title('Direct evaluate.py Benchmark Results - Performance Comparison')
        ...
        plt.title('Direct evaluate.py Benchmark - F1 Score Ranking')
        plt.ylim(0, 1.1)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
